...
**Session 04 決策（2026-07-10）：Gemini 免費 model 一路踩雷（embedding 除役、chat 額度 0～20/天），改用 Groq 當 Render 預設 LLM**

**需在本機 + Render Dashboard 操作（無法代做）：**

1. **Groq** 建立 Key（https://console.groq.com/keys）→ Render：`LLM_PROVIDER=groq`、`GROQ_API_KEY`（見 `deploy/free-llm-cloud.md`）
2. 同一把 Groq Key → Dify 後台 Model Provider（OpenAI 相容 `https://api.groq.com/openai/v1`）
...
5. 驗收 https://ai-agent-tutorial.onrender.com/learn Step 2（Groq）與 Step 4（Dify）
...
已完成：Render、Supabase、/learn、本機 Ollama/Dify；Session 04 改用 groq provider（Gemini 免費層一路踩雷，保留為備用）。
請用繁體中文。

下一步：依 deploy/free-llm-cloud.md 與 deploy/dify-cloud-setup.md 在 Render 設 GROQ_API_KEY；
...
*最後更新：2026-07-10（Session 04：Gemini 免費層一路踩雷改用 Groq 作為 Render 預設 LLM；新增 Render keep-alive workflow；開發日誌簡化為一般工程風格）*
