...
**Session 04（2026-07-10）已完成：** Render/Supabase 自動喚醒、Groq 零預算真實 LLM（`/ask` 線上驗收通過）、開發日誌簡化為一般工程風格。

**接下來的主目標：線上 `/learn` Step 4（Dify）可用**

1. 同一把 Groq Key → Dify 後台 **Model Provider**（OpenAI 相容 `https://api.groq.com/openai/v1`，model `llama-3.3-70b-versatile`）
2. 本機跑 Dify（`docker compose up -d`，見 `deploy/dify-setup.md`）+ `.\scripts\setup-dify-tunnel.ps1` → 複製 Cloudflare Tunnel URL
3. Render 設 `DIFY_API_BASE=https://<tunnel-host>/v1`、`DIFY_API_KEY=app-...`（見 `deploy/dify-cloud-setup.md`）
4. 驗收 https://ai-agent-tutorial.onrender.com/learn Step 4，`/health` 應顯示 `dify_configured: true`

**需在本機 + Dify 後台操作（無法代做）：** 上述 1～3 都需要你在 Dify 網頁介面與本機終端機操作。

**可選：** Oracle Cloud 24/7 自架 Dify，取代「本機 + 會變動的 Cloudflare Quick Tunnel」（`deploy/dify-cloud-setup.md` 進階章節）。
...
已完成：Render、Supabase、/learn、本機 Ollama/Dify；Session 04 改用 groq provider 並線上驗收通過（Gemini 免費層一路踩雷，保留為備用）；Render/Supabase 自動喚醒 workflow 已加。
請用繁體中文。

下一步：Dify 上線（Step 4）。同一把 Groq Key 設進 Dify Model Provider，
本機開 Tunnel 後在 Render 設 DIFY_API_BASE/DIFY_API_KEY，驗收線上 Step 4。
```
...
*最後更新：2026-07-10（Session 04：Groq 已上線驗收通過；Render/Supabase 自動喚醒已加；下一步為 Dify 上線）*
