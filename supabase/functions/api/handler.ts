...
const DOCS_URL =
  "https://github.com/frobel0520/AI-Agent-Tutorial/tree/main/docs";
...
export function createApiHandler(
  ...
  function env(name: string, fallback = ""): string {
    return (readEnv(name) ?? fallback).trim();
  }
  ...
  function bearerToken(request: Request): string {
    ...
    if (!match?.[1]?.trim()) {
      throw new HttpError(401, "Login is required to use this API.");
    }
    return match[1].trim();
  }
  ...
  async function requireModelAccess(userId: string): Promise<void> {
    if (!await difyAccessEnabled(userId)) {
      throw new HttpError(
        403,
        "Your account is not authorized to use hosted models.",
      );
    }
  }
  ...
  function corsHeaders(request: Request): HeadersInit {
    ...
    if (configuredOrigins.length > 0) {
      allowOrigin = origin
        ? configuredOrigins.includes(origin) ? origin : "null"
        : configuredOrigins[0];
    }

    return {
      "Access-Control-Allow-Headers":
        "authorization, x-client-info, apikey, content-type, x-event-type, x-webhook-signature",
  ...
  async function listNotes(userId: string): Promise<NoteRow[]> {
    const { data, error } = await supabase
      .from("notes")
      .select(NOTE_COLUMNS)
      .eq("user_id", userId)
  ...
  async function findNote(userId: string, noteId: number): Promise<NoteRow> {
    const { data, error } = await supabase
      .from("notes")
      .select(NOTE_COLUMNS)
      .eq("user_id", userId)
  ...
  async function createNote(
    ...
    await dispatchEvent(userId, "note.created", {
      note_id: note.id,
      title: note.title,
    });
  ...
  async function updateNote(
    ...
    const note = data as NoteRow;
    await dispatchEvent(userId, "note.updated", {
      note_id: note.id,
      title: note.title,
    });
  ...
  async function deleteNote(
    ...
    await findNote(userId, noteId);
    const { error } = await supabase.from("notes").delete().eq(
      "user_id",
      userId,
    )
      .eq("id", noteId);
    ...
    await dispatchEvent(userId, "note.deleted", { note_id: noteId });
  ...
  function selectRelevantNotes(
    ...
    return notes
      ...
      .sort((left, right) =>
        right.score - left.score || right.note.id - left.note.id
      )
      .slice(0, topK)
      .map(({ note }) => note);
  }
  ...
  function requireConfiguration(name: string): string {
    ...
    if (!value) {
      throw new HttpError(
        503,
        `${name} is required for the configured provider.`,
      );
    }
    return value;
  }
  ...
  async function callGemini(
    ...
    const parts = Array.isArray(candidates)
      ? ((candidates[0] as JsonObject | undefined)?.content as
        | JsonObject
        | undefined)?.parts
      : undefined;
  ...
  async function generateAnswer(
    ...
    if (provider === "ollama") {
      throw new HttpError(
        503,
        "Ollama is local-only and cannot run inside a hosted Edge Function.",
      );
    }
  ...
  function validateWebhookUrl(value: string): void {
    ...
    try {
      ...
      throw new HttpError(422, "Invalid webhook URL.");
    }
    const allowed = env("WEBHOOK_ALLOWED_URLS").split(",").map((v) => v.trim())
      .filter(Boolean);
    if (!allowed.length) {
      throw new HttpError(503, "Webhook delivery is not configured.");
    }
    if (
      url.protocol !== "https:" || url.username || url.password || url.hash ||
      (url.port && url.port !== "443") || !allowed.includes(url.href)
    ) {
      throw new HttpError(
        422,
        "Webhook URL is not an approved HTTPS destination.",
      );
    }
  }

  async function consumeModelQuota(userId: string): Promise<void> {
    const { data, error } = await supabase.rpc("consume_model_quota", {
      p_user_id: userId,
    });
    if (error) databaseError(error, "model quota check");
    if (data !== true) {
      throw new HttpError(
        429,
        "Hourly model request limit reached. Try again later.",
      );
    }
  }
  ...
  async function receiveIncomingWebhook(
    request: Request,
  ): Promise<RouteResult> {
    ...
    const receivedSignature =
      request.headers.get("X-Webhook-Signature")?.trim().toLowerCase() ?? "";
    ...
    // Owner is covered by the verified HMAC, never accepted from unsigned headers.
    ...
    if (
      ...
      throw new HttpError(
        422,
        "Signed webhook body must contain a valid user_id.",
      );
    }
    const sourceEventType = request.headers.get("X-Event-Type")?.trim() ||
      "unknown";
    const eventPayload = {
      body,
  ...
  async function dispatchEvent(
    ...
    const eventResult = await supabase
      .from("event_logs")
      .insert({ user_id: userId, event_type: eventType, payload: serialized })
      .select("id,created_at")
      .single();
    if (eventResult.error) {
    ...
    for (
      const subscription
        of (subscriptionsResult.data ?? []) as WebhookSubscriptionRow[]
    ) {
      ...
      try {
        ...
        const response = await fetchWithTimeout(
          subscription.url,
          { method: "POST", headers, body: bodyText, redirect: "error" },
          WEBHOOK_TIMEOUT_MS,
        );
        statusCode = response.status;
        responseBody = (await response.text()).slice(
          0,
          MAX_WEBHOOK_RESPONSE_BODY,
        );
  ...
  async function askQuestion(
    ...
    if (provider !== "mock") await requireModelAccess(userId);
    await consumeModelQuota(userId);
    const answer = await generateAnswer(
      provider,
      question,
      formatContext(sources),
    );
    await dispatchEvent(userId, "ask.completed", {
      provider,
      question,
  ...
  async function listWebhooks(userId: string): Promise<RouteResult> {
    const { data, error } = await supabase
      .from("webhook_subscriptions")
  ...
  async function createWebhook(
    ...
    const { data, error } = await supabase
      .from("webhook_subscriptions")
      .insert({ user_id: userId, event_types: eventTypes, secret, url })
      .select("id,url,event_types,created_at")
      .single();
    if (error) {
  ...
  async function deleteWebhook(
    ...
    const { error } = await supabase.from("webhook_subscriptions").delete().eq(
      "user_id",
      userId,
    )
      .eq("id", webhookId);
  ...
  async function listEvents(userId: string): Promise<RouteResult> {
    const { data, error } = await supabase
      .from("event_logs")
  ...
  async function askDify(
    ...
    await requireModelAccess(userId);
    await consumeModelQuota(userId);
    const body = await readJsonBody(request);
    const question = requiredString(body, "question");
    const user = userId;
    ...
    await dispatchEvent(userId, "dify.ask.completed", {
      provider: "dify",
      question,
    });
  ...
  async function routeRequest(request: Request): Promise<RouteResult> {
    ...
    if (
      (route.length === 0 || route.join("/") === "health") &&
      request.method === "GET"
    ) {
      const provider = configuredProvider();
    ...
    if (route.join("/") === "dify/access" && request.method === "GET") {
      return readDifyAccess(request);
    }
    if (route.join("/") === "hooks/incoming" && request.method === "POST") {
      return receiveIncomingWebhook(request);
    }
    const { id: userId } = await authenticatedUser(request);

    if (resource === "notes" && route.length <= 2) {
      if (!identifier && request.method === "GET") {
        return { status: 200, body: await listNotes(userId) };
      }
      if (!identifier && request.method === "POST") {
        return createNote(userId, request);
      }
      if (!identifier) {
      ...
      if (!action && request.method === "GET") {
        return { status: 200, body: await findNote(userId, noteId) };
      }
      if (!action && request.method === "PUT") {
        return updateNote(userId, request, noteId);
      }
    ...
    if (resource === "ask" && !identifier && request.method === "POST") {
      return askQuestion(userId, request);
    }

    if (resource === "webhooks" && route.length <= 2) {
      if (!identifier && request.method === "GET") {
        return listWebhooks(userId);
      }
      if (!identifier && request.method === "POST") {
        return createWebhook(userId, request);
      }
      if (identifier && request.method === "DELETE") {
        return deleteWebhook(userId, parseId(identifier));
      }
    }
    ...
    if (route.join("/") === "dify/ask" && request.method === "POST") {
      return askDify(userId, request);
    }
